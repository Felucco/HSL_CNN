				...
				conv_channel_for:for(ch=0; ch<IN_SHAPE_CH; ch += 1)
				{
					conv_ker_row_for:for(k_row = 0; k_row < KERNEL_SIZE; k_row ++)
					{
						...
						conv_ker_col_for:for(k_col = 0; k_col < KERNEL_SIZE; k_col ++)
						{
							...
							sum += in[out_row+k_row][out_col+k_col][ch] * weight[k_row][k_col][ch][filt];
						}
					}
				}
...
template<uint32_t IN_SHAPE_H, uint32_t IN_SHAPE_W, uint32_t IN_SHAPE_CH, uint8_t KERNEL_SIZE, uint8_t FILTERS>
void conv2(float out[IN_SHAPE_H-KERNEL_SIZE+1][IN_SHAPE_W-KERNEL_SIZE+1][FILTERS], float in[IN_SHAPE_H][IN_SHAPE_W][IN_SHAPE_CH], float weight[KERNEL_SIZE][KERNEL_SIZE][IN_SHAPE_CH][FILTERS], float bias[FILTERS])
{
